    ...
    const onSubmitReviewPress = () => {      
        ...
        if (user) {       
          ...
          reviewsRef.add({
            ...
            userID: data.id,
            rating: route.params.loocateRating,  //add in star rating variable when I get around to it...maybe just reference the one from toilets, but that is google...hmm.
            })
            
          setExistingReviewsArray([...existingReviewsArray , {title: review, address: route.params.address, toiletID: route.params.id, 
            userID: data.id, rating: route.params.loocateRating}]); 
        })
